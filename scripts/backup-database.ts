...
// Only a newly created private backup copy may be normalized. Never used by db:verify.
function normalizeBackupCopy(partialPath: string): void {
  const copy = new Database(partialPath, { fileMustExist: true });
  try {
    if (copy.pragma("journal_mode = DELETE", { simple: true }) !== "delete") {
      throw new Error("Could not normalize backup journal mode");
    }
  } finally { copy.close(); }
}
...
export async function createDatabaseBackup(
  ...
  normalizeBackupCopy(partialPath);
