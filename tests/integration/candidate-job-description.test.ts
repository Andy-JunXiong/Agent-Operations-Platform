...
it("shares JD CAS and atomic receipts across Web and MCP without impersonating either channel", () => {
  const w = fixture();
  const web = new WorkspaceService(w.database, { ...w.identity, channel: "WEB", requestId: "web-jd" });
  const input = { jd: "Complete web JD", sourceUrl: w.c.sourceUrl!, expectedCandidateVersion: 1,
    expectedJdHash: null as string | null, fullTextProvided: true, intentKey: randomUUID() };
  expect(() => w.service.candidateJobDescriptionService.recordFromWeb(w.c.id, input)).toThrow(/Web context/);
  expect(() => web.candidateJobDescriptionService.recordFromWeb(w.c.id, {...input, userConfirmed:true})).toThrow(/Invalid/);
  const first = web.candidateJobDescriptionService.recordFromWeb(w.c.id, input);
  expect(first.attribution.channel).toBe("WEB");
  expect(() => w.record(w.request())).toThrow(/changed/);
  const second = w.record({...w.request(), expectedJdHash:first.jdHash});
  expect(() => web.candidateJobDescriptionService.recordFromWeb(w.c.id, {...input, expectedJdHash:first.jdHash, intentKey:randomUUID()})).toThrow(/changed/);
  expect(web.candidateJobDescriptionService.recordFromWeb(w.c.id, input)).toEqual({...first,replayed:true});
  expect(w.service.jobLibraryService.description(w.c.id)?.jd_text).toBe(second.jd.text);
  w.database.exec("CREATE TRIGGER reject_web_jd_receipt BEFORE INSERT ON idempotency_records WHEN NEW.operation='candidate.jd.web.record' BEGIN SELECT RAISE(ABORT,'receipt unavailable'); END");
  expect(() => web.candidateJobDescriptionService.recordFromWeb(w.c.id, {...input, expectedJdHash:second.jdHash, intentKey:randomUUID()})).toThrow(/receipt unavailable/);
  expect(w.service.jobLibraryService.description(w.c.id)?.jd_text).toBe(second.jd.text);
});
