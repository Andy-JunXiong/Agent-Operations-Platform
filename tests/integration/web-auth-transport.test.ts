...
it("allows only CSRF-authorized library and candidate decisions with general browser writes disabled",async()=>{
  ...
  const jdInput={jd:"Saved source JD",sourceUrl:"https://www.seek.com.au/job/123",expectedCandidateVersion:2,
    expectedJdHash:null,fullTextProvided:true,intentKey:randomUUID()};
  expect((await w.request(jdPath,{method:"POST",headers,body:JSON.stringify(jdInput)})).status).toBe(200);
  const replay=await w.request(jdPath,{method:"POST",headers,body:JSON.stringify(jdInput)});
  expect(await replay.json()).toMatchObject({replayed:true,attribution:{channel:"WEB"}});
  expect((await w.request(jdPath,{method:"POST",headers,body:JSON.stringify({...jdInput,jd:"Changed same intent"})})).status).toBe(409);
  expect((await w.request(jdPath,{method:"POST",headers,body:JSON.stringify({...jdInput,intentKey:randomUUID()})})).status).toBe(409);
  expect((await w.request(jdPath,{method:"POST",headers,body:JSON.stringify({jd:"Old client",sourceUrl:jdInput.sourceUrl})})).status).toBe(422);
  expect((await w.request(jdPath,{method:"POST",headers:{cookie,"content-type":"application/json"},body:JSON.stringify(jdInput)})).status).toBe(403);
  ...
  expect(candidatePage).toContain('name="expectedCandidateVersion" value="2"');
  expect(candidatePage).toContain('name="expectedJdHash"');
  expect(candidatePage).toContain('name="fullTextProvided" value="true" required');
...
it.each([false, true])("imports scoped Platform Watch reports with general writes enabled=%s", async (writesEnabled) => {
  ...
  expect(listPage).toContain("平台下一步，哪些值得改？");
...
it("creates and exports scoped job resume variants without modifying the base or accepting submission",async()=>{
  ...
  const {csrfToken}=await (await w.request("/api/v1/session",{headers:{cookie}})).json();
  const headers={cookie,origin:webOrigin,"x-csrf-token":csrfToken,"content-type":"application/json"};
  const input={name:"ExampleCorp 中文",targetType:"APPLICATION",targetId:w.projectId,expectedBaseVersion:1,intentKey:randomUUID()},path="/api/v1/job-search/resume/variants";
  expect((await w.request(path)).status).toBe(401);
  ...
  const draft={...content,summary:"For ExampleCorp only"};
  ...
  try{
    ...
    expect(file.status).toBe(200);expect(file.headers.get("content-disposition")).toContain("ExampleCorp%20%E4%B8%AD%E6%96%87-v2.pdf");
    expect(exporter).toHaveBeenCalledWith(expect.objectContaining({content:draft}),"pdf");
    ...
    expect(preview.status).toBe(200);expect(w.service.resumeService.get(id)?.content.summary).toBe("For ExampleCorp only");
