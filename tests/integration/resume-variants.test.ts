...
it("keeps named application/candidate copies independent through persistence, order, previews and export",()=>{
  ...
  try{
    ...
    const candidate=w.service.candidateService.recordCandidate({provider:"seek",postingId:"variant",company:"ExampleCorp",title:"Engineer",role:"Engineer",authority:{type:"EXPLICIT_USER_DEV",confirmed:true,reference:"Test"},idempotencyKey:randomUUID()}).candidate;
    const input={name:"ExampleCorp AI",targetType:"APPLICATION",targetId:w.projectId,expectedBaseVersion:1,intentKey:randomUUID()};
    const first=service.createVariant(input),id=first.variant!.id;
    const second=service.createVariant({...input,name:"ExampleCorp Alternate",targetType:"CANDIDATE",targetId:candidate.id,intentKey:randomUUID()});
    ...
    expect(reloaded.exportSnapshot(2,id)).toMatchObject({content:edited,name:"ExampleCorp AI"});
    ...
    const html=resumeView(w.service,id);expect(html).toContain(`data-api-path="/variants/${id}"`);expect(html).toContain('data-filename="ExampleCorp AI"');
