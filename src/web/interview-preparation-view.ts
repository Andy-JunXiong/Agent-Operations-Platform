...
export function interviewPreparationPanel(service: WorkspaceService, projectId: string, query: unknown, zone: string) {
  ...
  const questions = saved.report.questions.map(q => `<article class="evidence-row"><h3>${e(q.question)}</h3><p class="muted">${{ EVIDENCED: "有来源依据", PARTIAL: "部分依据 · 需补充", UNKNOWN: "待确认依据" }[q.support]}</p>${q.jdQuote ? `<h4>岗位原文</h4><blockquote>${e(q.jdQuote)}</blockquote>` : '<p class="muted">通用练习题</p>'}${q.evidence.map(c => {
