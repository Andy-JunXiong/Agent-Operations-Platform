...
document.addEventListener('submit', async (event) => {
  ...
  if (form.matches('[data-library-source]')) body = {...values, sourceKey: values.sourceKey || `manual:${crypto.randomUUID()}`, sourceUrl: values.sourceUrl || null, expectedVersion: Number(values.expectedVersion)};
    ...
    if (form.matches('[data-library-jd]')) {
      body = {...body, expectedCandidateVersion: Number(values.expectedCandidateVersion),
        expectedJdHash: values.expectedJdHash || null, fullTextProvided: values.fullTextProvided === 'true'};
      // Ambiguous failures retry the same intent; edited input starts a new intent.
      const signature = JSON.stringify(body);
      if (form.dataset.jdPayload !== signature) {
        form.dataset.jdPayload = signature;
        form.dataset.jdIntent = crypto.randomUUID();
      }
      body.intentKey = form.dataset.jdIntent;
    }
  } else {
