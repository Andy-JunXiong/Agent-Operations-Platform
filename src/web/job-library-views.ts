...
export function fitPanel(service:WorkspaceService,id:string,sourceUrl:string|null,matchingEnabled=false){
  ...
  <form ${matchingEnabled?"data-library-compare":"data-library-jd"} data-candidate-id="${e(id)}" class="library-form">
  ${matchingEnabled?"":`<input type="hidden" name="expectedCandidateVersion" value="${service.jobSearchQueryService.getCandidate(id).recordVersion}"><input type="hidden" name="expectedJdHash" value="${description?.jd_text.trim()?canonicalHash({text:description.jd_text,sourceUrl:description.source_url}):""}">`}
  <label>职位来源链接<input type="url" name="sourceUrl" ${matchingEnabled?"":"required"} maxlength="2000" value="${e(description?.source_url??fit?.jd_source_url??sourceUrl??"")}"></label><label>完整职位描述<textarea name="jd" required minlength="${matchingEnabled?200:1}" maxlength="50000" rows="10">${e(description?.jd_text??fit?.jd_text??"")}</textarea></label>
  ${matchingEnabled?"":'<label><input type="checkbox" name="fullTextProvided" value="true" required>我确认保存的是完整 JD，而不是摘要。</label>'}
  <button class="button primary" ${matchingEnabled&&!snapshot.sources.length?"disabled":""}>${matchingEnabled?"匹配背景并准备简历":"保存 JD"}</button><p data-library-result role="status"></p></form>
