export const updateSkillLibraryPrompt = `请使用 Agent Operations Platform（连接名称可能仍为 Personal AI Workspace） 更新我的统一技能与项目库。本消息授权保存有依据的增量更新，不确认未知个人事实。
...
export const dailyGithubUpdatePrompt = `请使用 Agent Operations Platform（连接名称可能仍为 Personal AI Workspace） 执行每日 GitHub 项目检查与受影响技能更新。本任务授权检查已登记公开仓库、保存选定文件证据及有依据的技能增量更新；不授权修改仓库、确认未知事实或改变职位/申请/任务状态。
