...
export const webCandidateJobDescriptionSchema = z.object({
  jd: recordCandidateJobDescriptionSchema.shape.text,
  sourceUrl: recordCandidateJobDescriptionSchema.shape.sourceUrl,
  expectedCandidateVersion: recordCandidateJobDescriptionSchema.shape.expectedCandidateVersion,
  expectedJdHash: recordCandidateJobDescriptionSchema.shape.expectedJdHash,
  fullTextProvided: z.literal(true),
  intentKey: z.uuid(),
}).strict();
type DescriptionInput = z.infer<typeof recordCandidateJobDescriptionSchema>;
interface DescriptionReceipt {
  candidateId: string; candidateVersion: number; jdHash: string;
  jd: { text: string; sourceUrl: string };
  attribution: { principalId: string; channel: "MCP" | "WEB"; authorityReference: string;
...
export class CandidateJobDescriptionService {
  ...
  record(input: unknown): DescriptionReceipt {
    ...
    return this.persist(parsed, OPERATION);
  }

  // Only the authenticated, same-origin CSRF-protected Web route calls this entry.
  recordFromWeb(candidateId: string, input: unknown): DescriptionReceipt {
    const parsed = parseAssessment(webCandidateJobDescriptionSchema, input);
    if (this.identity().channel !== "WEB") throw new AuthorizationError("JD Web save requires an authenticated Web context");
    return this.persist(parseAssessment(recordCandidateJobDescriptionSchema, {
      candidateId, text: parsed.jd, sourceUrl: parsed.sourceUrl,
      expectedCandidateVersion: parsed.expectedCandidateVersion, expectedJdHash: parsed.expectedJdHash,
      fullTextProvided: parsed.fullTextProvided, userConfirmed: true,
      provenanceReference: "User-provided complete JD through authenticated website",
      authorityReference: `Authenticated website JD save intent ${parsed.intentKey}`,
      idempotencyKey: parsed.intentKey,
    }), "candidate.jd.web.record");
  }

  private persist(parsed: DescriptionInput, operation: string): DescriptionReceipt {
    const identity = this.identity();
    // Keep legacy MCP hashing and receipts unchanged. Web intents have their own namespace.
    ...
    return this.db.transaction(() => {
      ...
      const receipt: DescriptionReceipt = { candidateId: candidate.id, candidateVersion: candidate.recordVersion,
        jdHash: canonicalHash(jd), jd, attribution: { principalId: identity.principalId, channel: identity.channel,
